...
pub fn print_help() {
    ...
    println!("  {:<12} - Create a new Nula project", "create <name>".bright_yellow());
...
pub fn print_parse_error(err: &pest::error::Error<crate::parser::Rule>, path: &str, code: &str) {
...
pub fn print_generating_asm(file: &str) {
    println!("{} {}", "Generating assembly for".bold().bright_green(), Path::new(file).display().to_string().bright_green());
}

pub fn print_generating_llvm(file: &str) {
    println!("{} {}", "Generating LLVM IR for".bold().bright_green(), Path::new(file).display().to_string().bright_green());
}

pub fn print_assembling(file: &str) {
    println!("{} {}", "Assembling".bold().bright_blue(), Path::new(file).display().to_string().bright_blue());
}

pub fn print_linking(file: &str) {
    println!("{} {}", "Linking".bold().bright_magenta(), Path::new(file).display().to_string().bright_magenta());
}
